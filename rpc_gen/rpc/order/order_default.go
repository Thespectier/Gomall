...
func ModifyOrder(ctx context.Context, req *order.ModifyOrderReq, callOptions ...callopt.Option) (resp *order.ModifyOrderResp, err error) {
	resp, err = defaultClient.ModifyOrder(ctx, req, callOptions...)
	if err != nil {
		klog.CtxErrorf(ctx, "ModifyOrder call failed,err =%+v", err)
		return nil, err
	}
	return resp, nil
}
