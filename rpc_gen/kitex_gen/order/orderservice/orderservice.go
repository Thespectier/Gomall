...
var serviceMethods = map[string]kitex.MethodInfo{
	...
	"ModifyOrder": kitex.NewMethodInfo(
		modifyOrderHandler,
		newModifyOrderArgs,
		newModifyOrderResult,
		false,
		kitex.WithStreamingMode(kitex.StreamingUnary),
	),
...
func modifyOrderHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	switch s := arg.(type) {
	case *streaming.Args:
		st := s.Stream
		req := new(order.ModifyOrderReq)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(order.OrderService).ModifyOrder(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *ModifyOrderArgs:
		success, err := handler.(order.OrderService).ModifyOrder(ctx, s.Req)
		if err != nil {
			return err
		}
		realResult := result.(*ModifyOrderResult)
		realResult.Success = success
		return nil
	default:
		return errInvalidMessageType
...
func newModifyOrderArgs() interface{} {
	return &ModifyOrderArgs{}
}

func newModifyOrderResult() interface{} {
...
func (p *ModifyOrderArgs) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetReq() {
		p.Req = new(order.ModifyOrderReq)
	}
	return p.Req.FastRead(buf, _type, number)
}

func (p *ModifyOrderArgs) FastWrite(buf []byte) (n int) {
	if !p.IsSetReq() {
		return 0
	}
	return p.Req.FastWrite(buf)
...
func (p *ModifyOrderArgs) Size() (n int) {
	if !p.IsSetReq() {
		return 0
	}
	return p.Req.Size()
...
func (p *ModifyOrderArgs) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetReq() {
		return out, nil
	}
	return proto.Marshal(p.Req)
...
func (p *ModifyOrderArgs) Unmarshal(in []byte) error {
	msg := new(order.ModifyOrderReq)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Req = msg
...
var ModifyOrderArgs_Req_DEFAULT *order.ModifyOrderReq

func (p *ModifyOrderArgs) GetReq() *order.ModifyOrderReq {
	if !p.IsSetReq() {
		return ModifyOrderArgs_Req_DEFAULT
	}
	return p.Req
}

func (p *ModifyOrderArgs) IsSetReq() bool {
	return p.Req != nil
}

func (p *ModifyOrderArgs) GetFirstArgument() interface{} {
	return p.Req
}

type ModifyOrderResult struct {
	Success *order.ModifyOrderResp
}

var ModifyOrderResult_Success_DEFAULT *order.ModifyOrderResp

func (p *ModifyOrderResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(order.ModifyOrderResp)
	}
	return p.Success.FastRead(buf, _type, number)
}

func (p *ModifyOrderResult) FastWrite(buf []byte) (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.FastWrite(buf)
...
func (p *ModifyOrderResult) Size() (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.Size()
...
func (p *ModifyOrderResult) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetSuccess() {
		return out, nil
	}
	return proto.Marshal(p.Success)
...
func (p *ModifyOrderResult) Unmarshal(in []byte) error {
	msg := new(order.ModifyOrderResp)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *ModifyOrderResult) GetSuccess() *order.ModifyOrderResp {
	if !p.IsSetSuccess() {
		return ModifyOrderResult_Success_DEFAULT
	}
	return p.Success
}

func (p *ModifyOrderResult) SetSuccess(x interface{}) {
	p.Success = x.(*order.ModifyOrderResp)
}

func (p *ModifyOrderResult) IsSetSuccess() bool {
	return p.Success != nil
}

func (p *ModifyOrderResult) GetResult() interface{} {
	return p.Success
}
...
func (p *kClient) ModifyOrder(ctx context.Context, Req *order.ModifyOrderReq) (r *order.ModifyOrderResp, err error) {
	var _args ModifyOrderArgs
	_args.Req = Req
	var _result ModifyOrderResult
	if err = p.c.Call(ctx, "ModifyOrder", &_args, &_result); err != nil {
		return
	}
	return _result.GetSuccess(), nil
}
